...
access(all) resource BaseGameInstance {
    access(all) let id: UInt64
    access(all) let admin: Address
    access(all) var isOpen: Bool
    ...
    access(all) fun revealRound(secret: [UInt8]) {
        ...
        emit RoundRevealed(gameId: self.id, rolled: self.revealedOutcome!, beacon: self.beacon!, secret: self.revealedSecret!)
    }
    ...
    access(all) fun closeRound() {
        ...
        // Emit event with dice-specific data
        emit RoundClosed(gameId: self.id, outcome: self.revealedOutcome!, survivors: survivors)
    ...
    /// Evaluate dice guesses against the random roll
    access(all) fun evaluatePlayer(player: Address): Bool {
        let playerInput = self.players[player]?.getInputForRound(round: self.roundNumber)
        
        if playerInput == nil || self.lastRoll == nil {
            return false
        }
        
        // Cast the AnyStruct back to DiceInput
        let diceInput = playerInput as! DiceInput
        return diceInput.guessValue == self.lastRoll!
    }
...
access(all) contract MiniGame {
    ...
    access(all) fun createGame(): UInt64 {
        ...
        let newGame <- create GuessTheDiceInstance(id: gameId, admin: self.admin)
